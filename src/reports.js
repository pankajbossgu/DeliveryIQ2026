...
const { normalizeOrderId, normalizePaymentMode } = require('./upload');
...
class UniversalStore {
  ...
  async groupedReport(clientId, filters = {}) {
    ...
    const labels = { product: 'Product', product_category: 'Product Category', courier: 'Courier' };
    ...
    orders.forEach((order) => analyzeBy === 'courier' ? add(String(order.courier || 'Unmapped courier').trim().toLowerCase(), order, order.totalValue) : (order.products || []).forEach((line) => add(analyzeBy === 'product' ? String(line.normalizedProductName || line.originalProductName || 'Unmapped').trim().toLowerCase() : (line.productCategory || 'Unmapped'), order, line.rowValue)));
