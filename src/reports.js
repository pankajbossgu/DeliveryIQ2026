...
class UniversalStore {
  ...
  async syncCompletedReport(clientId, report, rows) {
    ...
    try { const groups = groupUniversalRows(rows); sync.counts.ordersProcessed = groups.size;
      ...
      } sync.status = 'completed'; sync.completedAt = new Date(); return sync;
    } catch (error) { return this.markFailed(clientId, report.reportId); }
  }
  async syncMongo(clientId, report, rows, emptyCounts) {
    ...
    try { const groups = groupUniversalRows(rows); const projections = [...groups.entries()].map(([id, orderRows]) => universalProjection(report, id, orderRows));
      ...
      return UniversalSync.findOneAndUpdate({ clientId, reportId: report.reportId }, { $set: { status: 'completed', completedAt: new Date(), error: null, counts } }, { new: true }).lean();
    } catch (error) { console.warn(JSON.stringify({ event: 'universal_sync_failed', reportId: report.reportId })); return this.markFailed(clientId, report.reportId); }
  }
}
class ReportStore { constructor({ mongoUri = process.env.MONGODB_URI, universalStore } = {}) { this.mongoUri = mongoUri; this.connection = null; this.memory = new Map(); this.rows = new Map(); this.universalStore = universalStore || new UniversalStore({ mongoUri }); } async synchronizeUniversal(clientId, report, rows) { try { return await this.universalStore.syncCompletedReport(clientId, report, rows); } catch (error) { console.warn(JSON.stringify({ event: 'universal_sync_failed', reportId: report.reportId })); if (typeof this.universalStore.markFailed === 'function') { try { return await this.universalStore.markFailed(clientId, report.reportId); } catch { /* The completed report remains authoritative if storage is unavailable. */ } } return { status: 'failed' }; } } async database() { if (!this.mongoUri || this.mongoUri.includes('127.0.0.1:27017/deliveryiq2026') && process.env.NODE_ENV === 'test') return null; if (!this.connection) this.connection = mongoose.connect(this.mongoUri, { serverSelectionTimeoutMS: 1500 }).catch(() => null); return this.connection; } async create(clientId, input) { const reportId = crypto.randomUUID(); const calculated = aggregate(input.rows, input.templateType); const dates = input.rows.map((row) => row.orderDate).filter(Boolean).sort(); const report = { reportId, clientId, requestId: input.requestId, reportName: input.reportName || `Delivery Report — ${dates[0] === dates.at(-1) ? dates[0] : `${dates[0]}–${dates.at(-1)}`}`, templateType: input.templateType, sourceFileName: input.sourceFileName, sourceRowCount: input.rows.length, uniqueOrderCount: calculated.totalOrders, reportStatus: 'completed', createdAt: new Date(), completedAt: new Date(), summary: calculated.summary, dateRange: { from: dates[0] || null, to: dates.at(-1) || null }, availableDimensions: dimensions(input.templateType), analytics: calculated.analytics }; if (await this.database()) { if (input.requestId) { const existing = await Report.findOne({ clientId, requestId: input.requestId }).lean(); if (existing) return existing; } await Report.create(report); const persistedRows = input.rows.map((row) => ({ reportId, clientId, orderId: row.originalOrderId || row.order_id, normalizedOrderId: row.normalizedOrderId || row.order_id, orderDate: row.order_date, category: row.category, originalStatus: row.originalStatus, normalizedStatus: row.normalizedStatus, originalProductName: row.originalProductName, normalizedProductName: row.normalizedProductName, masterCategory: row.masterCategory, productCategory: row.productCategory, paymentMode: row.payment_mode, courier: row.courier, orderSource: row.order_source, quantity: row.quantity, productPrice: row.productPrice, rowValue: row.rowValue })); await ReportRow.insertMany(persistedRows); await this.synchronizeUniversal(clientId, report, persistedRows); return report; } if (input.requestId) { const existing = [...this.memory.values()].find((item) => item.clientId === clientId && item.requestId === input.requestId); if (existing) return existing; } this.memory.set(reportId, report); const persistedRows = input.rows.map((row) => ({ orderId: row.originalOrderId || row.order_id, normalizedOrderId: row.normalizedOrderId || row.order_id, orderDate: row.order_date, category: row.category, originalStatus: row.originalStatus, normalizedStatus: row.normalizedStatus, originalProductName: row.originalProductName, normalizedProductName: row.normalizedProductName, masterCategory: row.masterCategory, productCategory: row.productCategory, paymentMode: row.payment_mode, courier: row.courier, orderSource: row.order_source, quantity: row.quantity, productPrice: row.productPrice, rowValue: row.rowValue })); this.rows.set(reportId, persistedRows); await this.synchronizeUniversal(clientId, report, persistedRows); return report; } async list(clientId) { if (await this.database()) return Report.find({ clientId, reportStatus: 'completed' }).sort({ createdAt: -1 }).lean(); return [...this.memory.values()].filter((report) => report.clientId === clientId).sort((a, b) => b.createdAt - a.createdAt); } async detail(clientId, reportId, filters = {}) { const report = await (await this.database() ? Report.findOne({ clientId, reportId }).lean() : this.memory.get(reportId)); if (!report || report.clientId !== clientId) return null; const allRows = await (await this.database() ? ReportRow.find({ clientId, reportId }).lean() : this.rows.get(reportId) || []); const rows = applyFilters(allRows, filters, report.templateType); return { ...report, filtered: aggregate(rows, report.templateType), filters: { availableDimensions: report.availableDimensions }, exportRows: rows }; } }
