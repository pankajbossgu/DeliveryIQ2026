...
const ALLOWED_ANALYZE_BY = ['product', 'product_category', 'courier'];
...
function invalidUniversal(response, message) { const status = message === 'analyzeBy is not recognized.' || message === 'Start date cannot be after the end date' ? 400 : 422; return response.status(status).json({ success: false, code: 'INVALID_UNIVERSAL_QUERY', message }); }
