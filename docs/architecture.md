...
- `GET /api/universal/summary` returns `{ success, summary }`, with current-order totals, total quantity/value, and counts by latest status category and actual status. It uses a client-scoped MongoDB aggregation rather than loading all orders into Node.js.
