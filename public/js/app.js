...
function modal({ title, description, destructive = false, content, submitText, cancelText = 'Cancel', onSubmit }) {
  ...
  const keydown = (event) => { if (event.key === 'Escape' && !destructive && !submit.disabled) { event.preventDefault(); closeModal(); } if (event.key === 'Enter' && document.activeElement?.matches('input')) { event.preventDefault(); submit.click(); } if (event.key !== 'Tab') return; const focusable = [...dialog.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex="-1"])')]; const first = focusable[0]; const last = focusable.at(-1); if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } };
